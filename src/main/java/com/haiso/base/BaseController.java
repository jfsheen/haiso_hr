package com.haiso.base;

import com.haiso.commons.utils.CookieUtils;
...
public abstract class BaseController extends CookieUtils {
    ...
    public void initWebRoot() {
        if (CommonsConstant.webRoot.equals("")) {
            CommonsConstant.webRoot = ContextLoader.getCurrentWebApplicationContext().getServletContext().getRealPath("/");
        }
    }

    public String getUploadedFilePath(HttpServletRequest request){
        if(!CommonsConstant.uploadedFilePath.equals("")){
            try{
                return request.getServletContext().getRealPath(CommonsConstant.uploadedFilePath);
            }catch  (Exception e){
    ...
    public String getMappingFilePath(HttpServletRequest request){
        if(!CommonsConstant.mappingFilePath.equals("")){
            try{
                return request.getServletContext().getRealPath(CommonsConstant.mappingFilePath);
            }catch  (Exception e){
                logger.error(e.getMessage());
            }
        }
